...
<head>
    ...
    <link rel="stylesheet" href="{{ asset('css/Approver/approval.css') }}">
</head>
<body >
    <x-sidebar></x-sidebar>
    <main>
    ...
    <div class="modal" id="orderModal">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <div class="revision-note" style="width: 100%; margin-bottom: 15px; display: none;">
                    <textarea id="revisionNote" placeholder="Enter revision notes here..." 
                    style="width: 100%; min-height: 100px; margin-bottom: 10px; padding: 8px; border: 1px solid #ddd; border-radius: 4px;"></textarea>
                </div>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        const btnRevisi = document.querySelector('.btn-secondary');
        const btnEdit = document.querySelector('.btn-primary');
        const btnTolak = document.querySelector('.btn-danger');
        const btnSetuju = document.querySelector('.btn-success');

        const revisionNoteDiv = document.querySelector('.revision-note');
        const revisionNote = document.getElementById('revisionNote')

        async function updateOrderStatus(noorder, status, note = null) {
            try {
                const response = await fetch(`/orders/${noorder}/update-status`, {
                    method: 'PUT',
                    headers: {
        ...
        btnRevisi.addEventListener('click', () => {
            if (revisionNoteDiv.style.display === 'none') {
                revisionNoteDiv.style.display = 'block';
                return;
            }
            ...
            updateOrderStatus(noorder, 'Revisi', note);
        });

        btnEdit.addEventListener('click', () => {
            const noorder = modalNoOrder.textContent;
            // Redirect to the edit page with the order number
            window.location.href = `/orders/${noorder}/edit`;
        });

        btnTolak.addEventListener('click', () => {
            const noorder = modalNoOrder.textContent;
            updateOrderStatus(noorder, 'Tolak');
        });

        btnSetuju.addEventListener('click', () => {
            const noorder = modalNoOrder.textContent;
            updateOrderStatus(noorder, 'Setuju');
        });

        modalClose.addEventListener('click', () => {
            modal.style.display = 'none';
            revisionNoteDiv.style.display = 'none';
            revisionNote.value = '';
        });
